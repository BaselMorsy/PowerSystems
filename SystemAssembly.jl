...
function Solve_OPF!(System ::System_Struct, method = "ACOPF", print_status = true)
    ...
    if method == "ACOPF"
            ...
            elseif method == "ACOPF-R"

                m = JuMP.Model(JuMP.with_optimizer(Ipopt.Optimizer, print_level =0))

                #Variables
                JuMP.@variable(m,e[i in Nodes_set])
                JuMP.@variable(m,f[i in Nodes_set])
                JuMP.@variable(m,V[i in Nodes_set])
                JuMP.@variable(m,pij[i in Nodes_set,j in Nodes_set])
                JuMP.@variable(m,qij[i in Nodes_set,j in Nodes_set ])
                JuMP.@variable(m, GenData[(GenData[:bus].==g), :Pmin][1,1] ≤ p[g in Gen_set] ≤ GenData[(GenData[:bus].==g), :Pmax][1,1])
                JuMP.@variable(m, GenData[(GenData[:bus].==g), :Qmin][1,1] ≤ q[g in Gen_set] ≤ GenData[(GenData[:bus].==g), :Qmax][1,1])
                ...
                JuMP.@NLconstraint(m,Pline[i in Nodes_set, j in Nodes_set],
                    pij[i,j] ==  -Sbase*((G[i,j])*V[i]^2) + Sbase*(((e[i]*e[j]+f[i]*f[j])*G[i,j]-(f[i]*e[j]-e[i]*f[j])*B[i,j])))

                JuMP.@NLconstraint(m, Qnodal[i in Nodes_set],
                    sum(qij[i,j] for j = Nodes_set)  ==  Sbase*0.5*sum(b[i,j] for j in Nodes_set)*V[i]^2 + sum(q[g] for g in Gen_set if GenData[(GenData[:bus].==g),:bus][1,1]==i) - BusData[i,:Qd])

                JuMP.@NLconstraint(m, Qline[i in Nodes_set,j in Nodes_set],
                    qij[i,j]
                    == Sbase*((B[i,j])*V[i]^2) +
                      Sbase*((-(e[i]*e[j]+f[i]*f[j])*B[i,j]-(f[j]*e[i]-e[j]*f[i])*G[i,j])))

                JuMP.@NLconstraint(m, V_mag_min[i in Nodes_set],
                    V[i] ≥ BusData[i, :Vmin])


                JuMP.@NLconstraint(m, V_mag_max[i in Nodes_set],
                    V[i] ≤ BusData[i, :Vmax])

                JuMP.@NLconstraint(m,v_mag[i in Nodes_set], V[i]^2 == e[i]^2+f[i]^2)
                JuMP.@NLconstraint(m,line_rating[i in Nodes_set, j in Nodes_set],
                    pij[i,j]^2+qij[i,j]^2 ≤ ((Sbase)^2)*S[i,j]^2)
                #OBJECTIVE
                ...
                JuMP.optimize!(m)
                    ...
                    println(ee)
                    println(ff)
                    δ = [atan(ff[i]/ee[i]) for i in Nodes_set]
                    Pd = System.BusData[!,3]
                    Qd = System.BusData[!,4]
                    ...
                    Pg = [i in System.Gen_Data[!,1] ? Pg.data[findall(x -> x==i,System.Gen_Data[!,1])][1,1] : 0 for i in 1:System.N_bus]
                    Qg = [i in System.Gen_Data[!,1] ? Qg.data[findall(x -> x==i,System.Gen_Data[!,1])][1,1] : 0 for i in 1:System.N_bus]
                    df = DataFrame(Bus = 1:System.N_bus, V = V.data, δ = δ,
                     Pg = Pg ,Qg = Qg, Pd = Pd, Qd = Qd)
                    System.BusData_output = df
...
function Solve_OTS!(System ::System_Struct,method = "ACOTS",use_dc_flag = true , print_status = true)
    ...
    if method == "ACOTS"
            ...
            elseif method =="ACOTS-R"
                M = maximum(S)^2;
                m = JuMP.Model(JuMP.with_optimizer(
                    Juniper.Optimizer;
                ...
                JuMP.@variable(m, GenData[(GenData[:bus].==g), :Pmin][1,1] ≤ p[g in Gen_set] ≤ GenData[(GenData[:bus].==g), :Pmax][1,1])
                JuMP.@variable(m, GenData[(GenData[:bus].==g), :Qmin][1,1] ≤ q[g in Gen_set] ≤ GenData[(GenData[:bus].==g), :Qmax][1,1])
                JuMP.@variable(m,s[i in Nodes_set,j in Nodes_set],Bin)

                #Constraints

                # 1.Nodal Constraints
                JuMP.@constraint(m,s_ones[i in Nodes_set,j in Nodes_set],s[i,j]==1)
                JuMP.@NLconstraint(m, Pnodal[i in Nodes_set],
                    sum(s[i,j]*pij[i,j] for j = Nodes_set) == sum(p[g] for g in Gen_set if GenData[(GenData[:bus].==g),:bus][1,1]==i) - BusData[i,:Pd])

                JuMP.@NLconstraint(m,Pline[i in Nodes_set],
                    sum(p[g] for g in Gen_set if GenData[(GenData[:bus].==g),:bus][1,1]==i ) - BusData[i,:Pd]
                    == sum(s[i,j]*(G[i,j])*V[i]^2 for j in Nodes_set) -
                     sum(s[i,j]*((e[i]*e[j]+f[i]*f[j])*G[i,j]+(f[i]*e[j]-e[i]*f[j])*B[i,j]) for j in Nodes_set))

                JuMP.@NLconstraint(m, Qnodal[i in Nodes_set],
                    sum(s[i,j]*qij[i,j] for j = Nodes_set)  == 0.5*sum(b[i,j] for j in Nodes_set)*V[i]^2 + sum(q[g] for g in Gen_set if GenData[(GenData[:bus].==g),:bus][1,1]==i) - BusData[i,:Qd])

                JuMP.@NLconstraint(m, Qline[i in Nodes_set],
                    0.5*sum(b[i,j] for j in Nodes_set)*V[i]^2 + sum(q[g] for g in Gen_set if GenData[(GenData[:bus].==g),:bus][1,1]==i) - BusData[i,:Qd]
                    == -sum(s[i,j]*(B[i,j]+b[i,j])*V[i]^2 for j = Nodes_set) +
                     sum(s[i,j]*((e[i]*e[j]+f[i]*f[j])*G[i,j]-(f[i]*e[j]-e[i]*f[j])*B[i,j]) for j in Nodes_set))

                JuMP.@NLconstraint(m, V_mag_min[i in Nodes_set],
                    e[i]^2+f[i]^2 ≥ BusData[i, :Vmin]^2)


                JuMP.@NLconstraint(m, V_mag_max[i in Nodes_set],
                    e[i]^2+f[i]^2 ≤ BusData[i, :Vmax]^2)

                #JuMP.@NLconstraint(m,line_rating[i in Nodes_set, j in Nodes_set],
                #    pij[i,j]^2+qij[i,j]^2 ≤ s[i,j]*((Sbase)^2)*S[i,j]^2+((1-s[i,j])*(Sbase)^2)*M)
                #OBJECTIVE
                ...
                JuMP.optimize!(m)
                    ...
                    line_status = zeros(size(System.LineLoading)[1],1)
                    for i in 1:size(System.LineLoading)[1]
                        line_status[i] = ceil(Z[System.Line_Constraints[i,1],System.Line_Constraints[i,2]])
                    end
